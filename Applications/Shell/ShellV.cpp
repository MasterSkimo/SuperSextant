...
#include <Applications/SextantBros/SextantBros.h>
extern bool warInit;
extern Souris mouse;
//Space Invaders
...
void ShellV::AfficherAide() {
	ecran->miniprintf("\t 'ls' => liste des activites disponibles\n");
	ecran->miniprintf("\t 'run appli' => Lance l'application appli.\n");
	ecran->miniprintf("\t 'ps' => liste des activites en cours\n");
//	ecran->miniprintf(
...
void ShellV::listerApplisEncours(ListeDouble<IntApp> *liste) {
	ecran->miniprintf(
			"Voici la liste des applications en cours de fonctionnement\n");
...
void ShellV::ls(String cmd) {
	...
	if (arg_1.isEqual("")) {
		...
		ecran->miniprintf(
				"ls retourne la liste des applications disponibles.\nIl n'a pas d'arguments\n");
	}
}
...
void ShellV::top(String cmd) {
	...
	if (arg_1.isEqual(0)) {
		...
		ecran->miniprintf(
				"top retourne la liste des applications en cours.\nIl n'a pas d'arguments.\n");
	}
}
...
void ShellV::runAppli(String cmd, int numThread) {
	...
	if (!arg_1.isEqual(0) && !arg_1.isEqual(vide)) {
		...
		if (arg_1.isEqual("SuperSextant")) {
			sem.P();

			//sched->addThread((thread*) getAppliAlancer("Compteur"), numThread);
			ecran->miniprintf("run lance Super Sextant Bros !!\n");

			PlateauSextantBros *plateauSextant;

			SextantBros *jeuP = new SextantBros(ecran, clavier, plateauSextant);
			jeuP->start("SextantBros");
			block.P(); // on bloque le shell
...
void ShellV::killAppli(String cmd) {
	...
	if (stringpid.isEqual("")) {
		ecran->miniprintf("PID demande\n");
	} else {
...
void ShellV::ps() {
	...
	for (int i = 0; i < MAX_THREAD; i++) {
		if (all_ready_thread[i].free == 0) {
			...
			String *mypid = new String(all_ready_thread[i].thread->pid);
			...
			ecran->miniprintf("%s Ready   Q Name %s | State : %s\n",
					...
					String::format(
							print_state(all_ready_thread[i].thread->state), 12)->getString());
		}
	}
	for (int i = 0; i < MAX_THREAD; i++) {
		if (all_waiting_thread[i].free == 0) {
			String *mypid = new String(all_waiting_thread[i].thread->pid);

			ecran->miniprintf("%s Waiting Q Name %s | State : %s\n",
					String::format(mypid->getString(), 3)->getString(),
					String::format(all_waiting_thread[i].thread->name, 10)->getString(),
					String::format(
							print_state(all_waiting_thread[i].thread->state),
							12)->getString());
